export const VRF_COORDINATOR: { [chainId: number]: string } = {
    //Rinkeby
    4: "0x6168499c0cFfCaCD319c818142124B7A15E857ab",
    //BNB Testnet
    97: "0x6A2AAd07396B36Fe02a22b33cf443582f682c82f",
    //Polygon Testnet
    80001: "0x7a1BaC17Ccc5b313516C5E16fb24f7659aA5ebed",

    //BNB Chain
    56: "0xc587d9053cd1118f25F645F9E08BB98c9712A4EE",
    //Polygon Mainnet
    137: "0xAE975071Be8F8eE67addBC1A82488F1C24858067",
}

export const LINK_TOKEN: { [chainId: number]: string } = {
    //Rinkeby
    4: "0x01BE23585060835E02B77ef475b0Cc51aA1e0709",
    //BNB Testnet
    97: "0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06",
    //Polygon Testnet
    80001: "0x326C977E6efc84E512bB9C30f76E30c160eD06FB",

    //BNB Chain
    56: "0x404460c6a5ede2d891e8297795264fde62adbb75",
    //Polygon Mainnet
    137: "0xb0897686c545045aFc77CF20eC7A532E3120E0F1",
}

export const GAS_LANE: { [chainId: number]: string } = {
    //Rinkeby
    4: "0xd89b2bf150e3b9e13446986e571fb9cab24b13cea0a43ea20a6049a85cc807cc",
    //BNB Testnet
    97: "0xd4bb89654db74673a187bd804519e65e3f71a52bc55f11da7601a13dcf505314",
    //Polygon Testnet
    80001: "0x4b09e658ed251bcafeebbc69400383d49f344ace09b9576fe248bb02c003fe9f",

    //BNB Chain
    56: "0xba6e730de88d94a5510ae6613898bfb0c3de5d16e609c5b7da808747125506f7",
    //Polygon Mainnet
    137: "0xcc294a196eeeb44da2888d17c0625cc88d70d9760a69d58d853ba6581a9ab0cd",
}